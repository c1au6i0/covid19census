...
Investigate the relation (if any) between influenza vaccination coverage and measures related to COVID-19 epidemic in Italy. Demigrafical and health related data of each italian region are also taken into account.
...
```{r dem_it_p}
dem_it_p <- vroom(
  "/Users/heverz/Documents/R_projects/covid19_interference/data/italy/pop_it.csv",
  col_types = cols(
...
```{r check}
apply(dem_it_wider[, 3:ncol(dem_it_wider)], 1, sum)
```
...
chronic_it_p <- vroom("/Users/heverz/Documents/R_projects/covid19_interference/data/italy/chronic_conditions.csv",
  col_types = cols(
    ITTER107 = col_character(),
    Territorio = col_character(),
...
```{r smoking_it, warning=FALSE, message=FALSE}
smoking_it <- vroom("/Users/heverz/Documents/R_projects/covid19_interference/data/italy/smoking.csv",
  col_types = cols(
    ITTER107 = col_character(),
    Territorio = col_character(),
    TIPO_DATO_AVQ = col_character(),
...
bweight_it <- vroom("/Users/heverz/Documents/R_projects/covid19_interference/data/italy/bweight_it.csv", col_types = cols(
  ITTER107 = col_character(),
  Territorio = col_character(),
  TIPO_DATO_AVQ = col_character(),
  `Tipo dato` = col_character(),
...
With this we check that the regions are the same across datasets. They are not always very consistent with names.
```{r check_join_all}
# check that the names are right otherwise when we join we loose data
# lapply(list(
#   cmr_it,
#   dem_it_wider,
#   regions_area,
...
#   bweight_it
# ), function(x) {
#   unique(x$region) %in% unique(dem_it_wider$region)
# })
```
...
# Join fl with cmr
fl_cmr_it <- inner_join(cmr_it, fl_it_2019, by = "region") %>%
  mutate_at(vars("perc_imm65","perc_imm"), as.numeric)


# join  with demographics
fl_cmr_dem_it <- inner_join(fl_cmr_it, dem_it_wider, by = "region")


# adding regions km2
fl_cmr_dem_it <- inner_join(fl_cmr_dem_it, regions_area, by = "region") %>%
...
# join health data
fl_cmr_dem_chronic_it <- inner_join(fl_cmr_dem_it, chronic_it, by = "region")

# join cancer data
fl_cmr_dem_health_cancer_it <- inner_join(fl_cmr_dem_chronic_it, cancer_it, by = "region")

# join smoking data
fl_cmr_dem_health_cancer_smoking_it <- inner_join(fl_cmr_dem_health_cancer_it, smoking_it, by = "region")

# join bweight
fl_cmr_dem_health_cancer_smoking_bweight_it <- inner_join(fl_cmr_dem_health_cancer_smoking_it, bweight_it, by = "region")

# something wrong with bweight


all_dat <- fl_cmr_dem_health_cancer_smoking_bweight_it
...
## Now We Rock`n`roll baby

Let' start looking at COVID-19 measures.

```{r}
unique(all_dat$COVID19_var)
```

### total_cases

```{r total_cases, fig.height= 6, fig.width=6}
all_dat %>% 
    filter(COVID19_var == "total_cases") %>% 
    ggplot(aes(date, value, group = region, color = region)) + 
      geom_line() +
      ...
      facet_wrap(vars(region)) +
        ...
        y = "confirmed cases") 
```
```{r total_cases_log, fig.height= 6, fig.width=6}
all_dat %>% 
    filter(COVID19_var == "total_cases") %>% 
    ggplot(aes(date, log2(value), group = region, color = region)) + 
      geom_line() +
      # geom_smooth(method = "lm", formula = y~x, se = FALSE) +
      scale_x_date(guide = guide_axis(check.overlap = TRUE, angle = 45)) +
      theme_bw() +
      ...
      facet_wrap(vars(region)) +
        labs(
        x = "date",
        y = "confirmed cases (log2)") 
```
Clearly at the extreme of date we lose linearity. But that might make sense


Let's do some smooth analysis.

```{r total_cases_smooth, fig.height= 6, fig.width=6, warning=FALSE}
p <- ggplotly(all_dat %>% 
    filter(COVID19_var == "total_cases") %>% 
    ggplot(aes(date, log2(value), group = region, color = region)) + 
      # geom_line() +
      geom_smooth(method = "lm", formula = y~x, se = FALSE, lwd = 0.5) +
      scale_x_date(guide = guide_axis(check.overlap = TRUE, angle = 45)) +
      theme_bw() +
      theme(legend.position = "none") +
      # facet_wrap(vars(region)) +
        ...
        x = "date",
        y = "confirmed cases"),
    tooltip = c("date", "log2(value)", "group")
...
Well beside `Piemonte` and `Molise`, rate is very similar.


othe vars:

cmr: altready linear. saturation hospitals
chr: funny at the beginning they were detecting only peple that were hospitalizing
cir:
```{r mortality_log, fig.height= 6, fig.width=6, message=FALSE, warning=FALSE}
all_dat %>% 
    filter(COVID19_var == "chr") %>% 
    ggplot(aes(date, value, group = region, color = region)) + 
      geom_line() +
      # geom_smooth(method = "lm", formula = y~x, se = FALSE) +
      scale_x_date(guide = guide_axis(check.overlap = TRUE, angle = 45)) +
...
<!-- This chunk below save datasets and data in my folder that then sync with dropbox. -->
```{r savedata, include=FALSE, eval=FALSE}
# files   ############
# report_workingprogress: notebook with all the progresses
# renv.lock: file to restore libraries
saveRDS(all_dat, "data/italy/all_dat.RDS")

drop_upload("data/italy/all_dat.RDS", path = "covid19/interference/data/")
drop_upload("tidy_data_script.R", path = "covid19/interference/code/")
drop_upload("data/italy/all_dat.RDS", path = "covid19/interference/data/italy/")
drop_upload("report_italy_wp.html", path = "covid19/interference/text/")
drop_upload("renv.lock", path = "covid19/interference/renv/")
drop_upload("report_italy_wp.Rmd", path = "covid19/interference/code/")
drop_upload("data/italy/NPI_it.xlsx", path = "covid19/interference/data/italy/raw/")
```
