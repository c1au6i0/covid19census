...
For **U.S** the prefix of the functions to extract data is `getus_`, and it is followed by the specific metric of interest:
  ...
  * `getus_covid`: extracts data of COVID-19 from the [New York Time github repository](https://github.com/nytimes/covid-19-data).
  ...
  * `getus_all`: executes all the above functions and join the results with other datasets statically contained in the package, and returns a `dataframe` with 314 variables.
...
Data exploration and modeling  can be conveniently performed on a (single) dataframe that contains COVID-19 as well as many other metrics retrieved from multiple sources. For example,  a correlation analysis of pair of selected U.S. variables is reported in  [figure \ref{fig:fig_corr}].
