...
```{r tab_info, results="asis", echo=FALSE}
tab1 <- as.data.frame(tribble(
   ~getus_all, ~getit_all,
   "304",   "64",
   "2790",  "21",
   "2020-01-21", "2020-02-24"
  
))

rownames(tab1) <- c("variables", "counties-regions", "from") 


print(xtable(tab1, caption = "Info regarding dataframes", label="tab:tab1"), comment=FALSE)
```

```{r fig_corr, fig.cap="COVID-19 Mortality and Obesity in Italy", results="asis", echo=FALSE, fig.height=4, fig.width=4, warning=FALSE, message=FALSE}

dat_it %>% 
  filter(date == max(date)) %>% 
  ggplot(aes(cmr, bweight_obese)) +
      geom_smooth(method = "lm", formula = y~x, se = FALSE, lwd = 0.8, colour = "gray35", lty = 4) +
        geom_label_repel(aes(label = region), label.size = 0.1, position = position_stack(vjust = 1), size = 3) +
        scale_x_continuous(labels = percent_format(scale = 1, accuracy = 1)) +
        scale_y_continuous(limits = c(6, 13), breaks = seq(6, 13, 1), labels = percent_format(scale = 1, accuracy = 1)) +
        geom_point(size = 3) +

    labs(
          title = "COVID-19 Mortality and Obesity",
          subtitle = paste0("date: ", max(dat_it$date)),
          x = "Obesity (% of regional population)",
